...
const RESIGNED_NAMES = new Set(["王煦晖", "郝锦", "符永涛"]);
...
export async function buildFinalReviewWorkspacePayload(user: SessionUser) {
  ...
  const employeeRows = employeeUsers.map((employee) => {
    ...
    // 排除已离职人员
    if (RESIGNED_NAMES.has(row.name)) return false;
    return true;
  });
  ...
  const totalSubjectCount = employeeUsers.length + leaderUsers.length;
  ...
  const companyDistributionOverviews = {
    withRoot: buildCompanyDistributionOverview(companyDistributionRows, totalSubjectCount),
    withoutRoot: buildCompanyDistributionOverview(companyDistributionRowsWithoutRoot, totalSubjectCount),
  };
